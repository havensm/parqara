...
function getMustDoChips(attraction: ParkCatalogDto["mustDoOptions"][number]) {
  const chips = [`Thrill ${attraction.thrillLevel}/5`];

  if (attraction.familyFriendly) {
    chips.push("Family favorite");
  } else if (attraction.kidFriendly) {
    chips.push("Kid-friendly");
  } else {
    chips.push("Signature ride");
  }
...
export function TripForm({ catalog, initialTrip }: { catalog: ParkCatalogDto; initialTrip: TripDetailDto }) {
  ...
  const allowedMustDoIds = useMemo(() => new Set(catalog.mustDoOptions.map((attraction) => attraction.id)), [catalog.mustDoOptions]);
  const initialValues = useMemo(() => {
    const baseValues = buildInitialValues(initialTrip, startsBlank);

    return {
      ...baseValues,
      mustDoRideIds: baseValues.mustDoRideIds.filter((rideId) => allowedMustDoIds.has(rideId)),
    };
  }, [allowedMustDoIds, initialTrip, startsBlank]);
        ...
        <div className="border-t border-slate-200 px-5 py-5 sm:px-6 sm:py-6">
          ...
          <section className="mt-8">
            ...
            {catalog.mustDoOptions.length ? (
                    ...
                    >
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {chips.map((chip) => (
                          <span
                            key={chip}
                            className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500"
                          >
                            {chip}
                          </span>
                        ))}
              ...
              <div className="mt-3 rounded-[24px] border border-dashed border-slate-200 bg-white px-4 py-4 text-sm text-slate-500">
                No marquee attractions are curated for this park yet.
              </div>
            )}
          </section>
